...
def main(batch_size, emb_size, hidden_size, learning_rate, epochs, verbosity):
    ...
    net = Model(len(vocabtoidx), len(labeltoidx), emb_size, hidden_size).to(device)
    ...
    print(classificationreport(correct,predicted,target_names=list(zip(*labeltoidx.items()))[0][:-1]))
